		...
		Select Case pTransactionType
			Case "VALIDATE" 'Custom type to validate address & cc by authorizing $0.01 then voiding it
				If Not IsNull(pCardID) And IsNull(pTransactionID) Then
					...
					cmd.Parameters.Append cmd.CreateParameter("OriginalID", adBigInt, adParamInput, , pAuthorizeID) 'Null value must come from variable
			...
			Case "CREDIT"
				If Not IsNull(pAuthorizeID) _
					...
					cmd.Parameters.Append cmd.CreateParameter("OriginalID", adBigInt, adParamInput, , pAuthorizeID)
